...
namespace SirRandoo.ToolkitUtils.Commands
{
    ...
    public class PawnBody : CommandBase
    {
        ...
        {
            string healthOverviewHeader = await "HealthOverview".TranslateSimpleAsync();

            if (!PurchaseHelper.TryGetPawn(context.User.Username, out Pawn pawn))
            {
                string noPawnMessage = await "TKUtils.NoPawn".TranslateSimpleAsync();

                await context.Reply(noPawnMessage.WithHeader(healthOverviewHeader));

                return;
            }

            var func = new Func<string>(() => GetPawnBody(context, pawn));
            string result = await func.OnMainThreadAsync();

            await context.Reply(result.WithHeader(healthOverviewHeader));
        }
        ...
        private static string GetPawnBody(IContext context, [NotNull] Pawn target)
        {
            ...
            {
                builder.Append(context.UseEmojis ? ResponseHelper.TemperatureGlyph : "ComfyTemperatureRange".TranslateSimple());
                builder.Append(" ");
            ...
            foreach (IGrouping<BodyPartRecord, Hediff> item in hediffsGrouped)
            {
                ...
                foreach (IGrouping<int, Hediff> group in item.GroupBy(h => h.UIGroupKey))
                {
                    ...
                    {
                        builder.Append(context.UseEmojis ? ResponseHelper.BandageGlyph : string.Empty);
                    }
