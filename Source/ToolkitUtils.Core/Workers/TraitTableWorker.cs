...
namespace SirRandoo.ToolkitUtils.Workers
{
    public class TraitTableWorker : TableWorker<TableSettingsItem<TraitItem>>
    {
        ...
        {
            if (UiHelper.TableHeader(_addStateHeaderRect, _addStateHeaderInnerRect, _addStateKey == StateKey.Enable ? Widgets.CheckboxOnTex : Widgets.CheckboxOffTex))
            {
                _addStateKey = _addStateKey == StateKey.Enable ? StateKey.Disable : StateKey.Enable;
                NotifyGlobalAddStateChanged(_addStateKey);
            }

            if (UiHelper.TableHeader(_removeStateHeaderRect, _removeStateHeaderInnerRect, _removeStateKey == StateKey.Enable ? Widgets.CheckboxOnTex : Widgets.CheckboxOffTex))
            {
            ...
            if (UiHelper.TableHeader(_expandedHeaderRect, _expandedHeaderInnerRect, Textures.Gear))
            {
        ...
        private protected void DrawSortableHeaders()
        {
            ...
            if (UiHelper.TableHeader(NameHeaderRect, NameHeaderTextRect, _nameHeaderText))
            {
                _sortKey = SortKey.Name;
                anyClicked = true;
            }

            if (UiHelper.TableHeader(AddPriceHeaderRect, AddPriceHeaderTextRect, _addPriceHeaderText))
            {
                _sortKey = SortKey.AddPrice;
                anyClicked = true;
            }

            if (UiHelper.TableHeader(RemovePriceHeaderRect, RemovePriceHeaderTextRect, _removePriceHeaderText))
            {
        ...
        protected virtual void DrawTrait(Rect canvas, [NotNull] TableSettingsItem<TraitItem> trait)
        {
            ...
            {
                UiHelper.IntEntry(addPriceRect, ref trait.Data.CostToAdd);
            }
            ...
            {
                UiHelper.IntEntry(removePriceRect, ref trait.Data.CostToRemove);
            }
        ...
        private void DrawConfigurableTraitName(Rect canvas, [NotNull] TableSettingsItem<TraitItem> trait)
        {
            ...
            if (UiHelper.FieldButton(canvas, trait.EditingName ? Widgets.CheckboxOffTex : Textures.Edit, trait.EditingName ? _closeTraitNameTooltip : _editTraitNameTooltip))
            {
        ...
        private void DrawLeftExpandedSettingsColumn(Rect canvas, [NotNull] ITableItem<TraitItem> trait)
        {
            ...
            if (Widgets.ButtonText(removeKarmaField, trait.Data.TraitData.KarmaTypeForRemoving == null ? _defaultKarmaTypeText : trait.Data.TraitData.KarmaTypeForRemoving.ToString()))
            {
