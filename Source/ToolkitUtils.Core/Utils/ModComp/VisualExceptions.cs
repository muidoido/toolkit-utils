    ...
    public static class VisualExceptions
    {
        ...
        static VisualExceptions()
        {
            ...
            try
            {
                ...
                HandleExceptionMethod = AccessTools.Method(ExceptionStateClass, "Handle", new[] { typeof(Exception) });
                Active = true;
            }
            catch (Exception e)
            {
                TkUtils.Logger.Error("Could not enable visual exception compatibility", e);
            }
        }
